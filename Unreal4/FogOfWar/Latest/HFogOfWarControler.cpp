...
void AHFogOfWarControler::LoadWorldSettings()
{
	...
	for (ULevel* Level : World->GetLevels())
	{
		...
		//WorldBounds += Level->LevelBoundsActor->CalculateLevelBounds()
	}
...
{
	PassedDynamicTexturePixels.Init(255, TextureSize * TextureSize);
	DynamicTexturePixels.Init(255, TextureSize * TextureSize);
	LastDynamicTexturePixels.Init(255, TextureSize * TextureSize);
	LastUpdateTime = 0;
}
...
bool AHFogOfWarControler::IsPositionVisible(const FVector& pos)
{
	...
	return DynamicTexturePixels[y * TextureSize + x] == 255;
}
...
void AHFogOfWarControler::UpdatePlayerPosition(const FVector& pos)
{
	...
	for (int x = minX; x < maxX; ++x)
	{
		for (int y = minY; y < maxY; ++y)
		{
			...
			{
				PassedDynamicTexturePixels[y * TextureSize + x] = 100;
			}
		}
	}
	...
	for (int x = minX; x < maxX; ++x)
	{
		for (int y = minY; y < maxY; ++y)
		{
			...
			if (distance < texelRadius)
			{
				DynamicTexturePixels[y * TextureSize + x] = 255;
			}
		}
	}
}
